...
impl RQuery {
    ...
    fn update(&mut self, primary_key: i64, columns: Vec<i64>) -> bool {
        ...
        let base_schema_encoding = result[self.table.page_range.base_container.schema_encoding_column as usize];
        let base_indirection_column = result[self.table.page_range.base_container.indirection_column as usize];
        ...
        if base_rid == base_indirection_column {
            ...
            if base_schema_encoding == 0 {
                let mut base_schema_encoding = addrs_base[self.table.page_range.base_container.schema_encoding_column as usize].page.lock().unwrap();
                base_schema_encoding.write(1);
            }
        } else {
            // second and subsequent updates
            let Some(existing_tail_record) = self.table.page_directory.get(&base_indirection_column) else {
                return false;
            };
            ...
            let mut lastest_tail_schema_encoding = addrs_existing[self.table.page_range.base_container.schema_encoding_column as usize].page.lock().unwrap();
        ...
        self.table.page_range.tail_container.insert_record(new_rid, values);

        // update the indirection column of the base record
        let mut indirection_base = addrs_base[self.table.page_range.base_container.indirection_column as usize].page.lock().unwrap();
