                ...
                match op.func {
                    QueryFunctions::Insert => {
                        ...
                        if let Some(pk) = op.primary_key {
                            ...
                            debug!(
                                "Rolling back insert by deleting record with primary key: {}",
                                pk
                            );
                            query.delete(pk);
                        }
                    ...
                    QueryFunctions::Update => {
                        ...
                        if let Some(pk) = op.primary_key {
                            if let Some(old_values) = &op.old_values {
                                ...
                                if !update_result {
                                    debug!(
                                        "Warning: Failed to roll back update for primary key: {}",
                                        pk
                                    );
                                }
                            }
                    ...
                    QueryFunctions::Delete => {
                        ...
                        if let Some(f) = first {
                            if let Some(a) = f {
                                ...
                                let record_before_delete = query.select(
                                    primary_key,
                                    0,
                                    vec![1; q.table.clone().table.read().unwrap().num_columns],
                                );
                    ...
                    QueryFunctions::Insert => {
                        ...
                        if insert_result {
                            ...
                            self.executed_operations.push(ExecutedOperation {
                                ...
                                table: t.clone(),
                                primary_key: if !args.is_empty() {
                                    Some(args[0])
                                } else {
                                    None
                                }, // Assuming first argument is primary key
                    ...
                    QueryFunctions::Update => {
                        ...
                        if let Some(f) = first {
                            if let Some(a) = f {
                                ...
                                let old_values = if let Some(records) = query.select(
                                    primary_key,
                                    0,
                                    vec![1; q.table.clone().table.read().unwrap().num_columns],
                                ) {
                                ...
                                // Execute the update
                                debug!(
                                    "Executing update for primary key: {} with values: {:?}",
                                    primary_key, rest
                                );
                                ...
                                if update_result {
                                    ...
                                    debug!(
                                        "Update operation failed for primary key: {}",
                                        primary_key
                                    );
                                    false
                                }
                        ...
                        if args_clone.len() == 2 {
                            ...
                            match (key, col) {
                                (Some(k), Some(c)) => {
                                    ...
                                    debug!(
                                        "Executing increment for primary key: {} on column: {}",
                                        k, c
                                    );
...
mod tests {
    ...
    fn abort_test() {
        ...
        assert!(
            rolled_back_record.is_some(),
            "Failed to read rolled back record"
        );
        ...
        if !orig_records.is_empty() && !rolled_records.is_empty() {
            ...
            let orig_first = orig_records
                .iter()
                .find(|r| r.is_some())
                .map(|r| r.as_ref().unwrap());
            let rolled_first = rolled_records
                .iter()
                .find(|r| r.is_some())
                .map(|r| r.as_ref().unwrap());
            ...
            if orig_first.is_some() && rolled_first.is_some() {
                ...
                assert_eq!(
                    orig_data.columns[0], rolled_data.columns[0],
                    "Primary key doesn't match after rollback"
                );
                assert_eq!(
                    orig_data.columns[1], rolled_data.columns[1],
                    "Column 1 not properly rolled back"
                );
                assert_eq!(
                    orig_data.columns[2], rolled_data.columns[2],
                    "Column 2 not properly rolled back"
                );
            } else {
    ...
    fn comprehensive_abort_test() {
        ...
        assert_eq!(
            initial_record.columns[0],
            Some(99),
            "Initial primary key doesn't match"
        );
        assert_eq!(
            initial_record.columns[1],
            Some(100),
            "Initial column 1 value doesn't match"
        );
        assert_eq!(
            initial_record.columns[2],
            Some(200),
            "Initial column 2 value doesn't match"
        );
        ...
        assert!(
            !updated_records.is_empty(),
            "Updated records vector is empty"
        );

        // Find the first valid record
        let updated_record_opt = updated_records.iter().find(|r| r.is_some());
        assert!(
            updated_record_opt.is_some(),
            "No valid updated record found"
        );
        ...
        assert_eq!(
            updated_record.columns[0],
            Some(99),
            "Updated primary key doesn't match"
        );
        assert_eq!(
            updated_record.columns[1],
            Some(555),
            "Updated column 1 value doesn't match expected 555"
        );
        assert_eq!(
            updated_record.columns[2],
            Some(666),
            "Updated column 2 value doesn't match expected 666"
        );
        ...
        assert!(
            ...
            "Failed to retrieve record after abort"
        );
        ...
        assert_eq!(
            final_record.columns[0],
            Some(99),
            "Final primary key doesn't match"
        );
        assert_eq!(
            final_record.columns[1],
            Some(100),
            "Column 1 not restored to original value 100"
        );
        assert_eq!(
            final_record.columns[2],
            Some(200),
            "Column 2 not restored to original value 200"
        );
    ...
    fn simplified_abort_test() {
        ...
        println!(
            "Step 1: Inserting record with values ({}, {}, {})",
            pk, initial_val_1, initial_val_2
        );
        ...
        println!(
            "Step 2: Updating record with values ({}, {}) to verify update works",
            updated_val_1, updated_val_2
        );
        let update_success =
            query.update(pk, vec![Some(pk), Some(updated_val_1), Some(updated_val_2)]);
        ...
        if let Some(record_values) = table.read(pk) {
            ...
            assert_eq!(
                record_values[NUM_RESERVED_COLUMNS as usize + 0],
                pk,
                "Primary key doesn't match after abort"
            );
            assert_eq!(
                record_values[NUM_RESERVED_COLUMNS as usize + 1],
                initial_val_1,
                "Column 1 not restored to {} (got {})",
                initial_val_1,
                record_values[NUM_RESERVED_COLUMNS as usize + 1]
            );
            assert_eq!(
                record_values[NUM_RESERVED_COLUMNS as usize + 2],
                initial_val_2,
                "Column 2 not restored to {} (got {})",
                initial_val_2,
                record_values[NUM_RESERVED_COLUMNS as usize + 2]
            );
        ...
        if final_record.is_some() {
            ...
            if !final_records.is_empty() {
                ...
                if first_record.is_some() {
                    ...
                    assert_eq!(
                        record_data.columns[0],
                        Some(1),
                        "Primary key mismatch after abort"
                    );
                    assert_eq!(
                        record_data.columns[1],
                        Some(100),
                        "Column 1 not restored to original value"
                    );
                    assert_eq!(
                        record_data.columns[2],
                        Some(200),
                        "Column 2 not restored to original value"
                    );
